...
// ============================================================================
// SAM'S HARDWARE SECURE API ENDPOINTS
// ============================================================================

// Sam's Hardware API Secrets - CONFIGURE THESE WITH YOUR ACTUAL VALUES
define('PAYFAST_MERCHANT_ID', '32932635');  // TODO: Replace with your sams PayFast Merchant ID
define('PAYFAST_MERCHANT_KEY', '2jwe3h1up0m5p');  // TODO: Replace with your sams PayFast Merchant Key
define('PAYFAST_PASSPHRASE', 'C0ff33_TaSt3s-B3tt3R_At-Sunr1s3');  // TODO: Replace with your sams PayFast Passphrase
define('BOBGO_API_KEY', '01a6abc327384b9489bbbb9425bf0981');  // TODO: Replace with SAMS bobgo API Key

// CORS Configuration - Only allow your React app domain
add_action('rest_api_init', function() {
    remove_filter('rest_pre_serve_request', 'rest_send_cors_headers');
    add_filter('rest_pre_serve_request', function($value) {
        // Define allowed origins for Sam's Hardware
        $allowed_origins = array(
            'https://www.samshardware.co.za',
            'https://samshardware.co.za',
            'https://api.samshardware.co.za',
			'http://localhost:3000',// API subdomain
        );
...
function verify_sams_hardware_request($request) {
    // Check API key
    $api_key = $request->get_header('X-API-Key');
    if ($api_key !== 'sams-hardware-react-2024') {
        error_log('[SAMS-HARDWARE] Invalid API key received');
    ...
    if ($requests > $rate_limit) {
        error_log('[SAMS-HARDWARE] Rate limit exceeded for IP ' . $ip . ' (' . $requests . '/' . $rate_limit . ')');
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('sams-hardware/v1', '/payments/create', array(
        'methods' => 'POST',
        'callback' => 'handle_payfast_payment',
        'permission_callback' => 'verify_sams_hardware_request',
    ));
    
    // BobGo shipping endpoint
    register_rest_route('sams-hardware/v1', '/shipping/calculate', array(
        'methods' => 'POST',
        'callback' => 'handle_bobgo_shipping',
        'permission_callback' => 'verify_sams_hardware_request',
    ));

    // Delivery options (Delivery Options by KaizenTech plugin)
    // GET  /wp-json/sams-hardware/v1/delivery/options
    // POST /wp-json/sams-hardware/v1/delivery/calculate
    register_rest_route('sams-hardware/v1', '/delivery/options', array(
        'methods'             => 'GET',
        'callback'            => 'ktdo_get_delivery_options_secure',
        'permission_callback' => 'verify_sams_hardware_request',
    ));

    register_rest_route('sams-hardware/v1', '/delivery/calculate', array(
        'methods'             => 'POST',
    ...
    register_rest_route('sams-hardware/v1', '/products', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/(?P<id>\d+)', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/(?P<id>\\d+)/variations', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/brands', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/categories', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/categories/children', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/orders/create', array(
        'methods' => 'POST',
    ...
    register_rest_route('sams-hardware/v1', '/orders', array(
        'methods' => 'POST',
        'callback' => 'create_order_secure',
        'permission_callback' => 'verify_sams_hardware_request',
    ));
    ...
    register_rest_route('sams-hardware/v1', '/custom-designs', array(
        'methods' => 'POST',
    ...
    register_rest_route('sams-hardware/v1', '/images/base64', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/orders/(?P<id>\d+)', array(
        'methods' => 'PUT',
    ...
    register_rest_route('sams-hardware/v1', '/cache/clear', array(
        'methods' => 'POST',
        'callback' => 'clear_sams_hardware_cache',
        'permission_callback' => 'verify_sams_hardware_request',
    ));
    
    // Debug logs endpoint (for debugging)
    register_rest_route('sams-hardware/v1', '/debug/logs', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/attributes', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/gallery/media', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/attributes/(?P<id>\\d+)/terms', array(
        'methods' => 'GET',
    ...
    register_rest_route('sams-hardware/v1', '/products/(?P<id>\\d+)/linked', array(
        'methods' => 'GET',
...
register_rest_route('sams-hardware/v1', '/validate-stock', array(
    'methods' => 'POST',
...
function clear_sams_hardware_cache($request) {
    global $wpdb;
    
    // Delete all sams_hardware transients (including linked products cache)
    $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_sams_hardware_%' OR option_name LIKE '_transient_timeout_sams_hardware_%'");
...
function get_debug_logs($request) {
    ...
    while (!$file->eof()) {
        $line = $file->current();
        if (strpos($line, 'Sams Hardware Debug') !== false) {
            $lines[] = trim($line);
        }
        $file->next();
    }
    ...
    try {
        ...
        foreach ($order_data['line_items'] as $item) {
            ...
            if ($is_stock_managed) {
                ...
                error_log("Sams Hardware Debug: Stock validated for product {$check_id}: Available: {$current_stock}, Required: {$quantity}");
            } else {
                 // Stock not managed - just check stock status
                 $stock_status_row = $wpdb->get_row($wpdb->prepare(
                    "SELECT meta_value FROM {$wpdb->postmeta} 
                ...
                if ($status === 'outofstock') {
                 ...
                 error_log("Sams Hardware Debug: Stock skipped for unmanaged product {$check_id} (Status: {$status})");
            }
        }
        }
        ...
        error_log("Sams Hardware Debug: Creating PayFast payment - Order ID: {$order_data['order_id']}, Amount: {$order_data['amount']}, Customer: {$customer_name}");
        ...
        error_log("Sams Hardware Debug: PayFast data prepared - Fields: " . implode(', ', array_keys($payfast_data)) . ", Signature: " . substr($signature, 0, 10) . "...");
        ...
        // Log the error
        error_log("Sams Hardware Debug: Stock validation failed - " . $e->getMessage());
...
// ============================================================================
// DELIVERY OPTIONS (Delivery Options by KaizenTech)
...
function ktdo_calculate_delivery_secure($request) {
    ...
    $option = function_exists('ktdo_get_option_by_id') ? ktdo_get_option_by_id($option_id) : null;
    ...
    return array(
        ...
        'option_id'      => $option_id,
        'cost'           => $result['cost'],
        'cost_formatted' => function_exists('wc_price') ? wc_price($result['cost']) : 'R' . number_format($result['cost'], 2),
...
function get_active_brand_taxonomy() {
    ...
    foreach ($possible_taxonomies as $taxonomy) {
        if (taxonomy_exists($taxonomy)) {
            ...
            error_log('Sams Hardware Debug: Found active brand taxonomy: ' . $taxonomy);
            return $active_taxonomy;
        }
    }
    ...
    error_log('Sams Hardware Debug: All product taxonomies: ' . print_r($all_taxonomies, true));
    
    // No brand taxonomy found
    $active_taxonomy = false;
    error_log('Sams Hardware Debug: No brand taxonomy found!');
    return $active_taxonomy;
}
...
function format_product_data($product) {
    ...
    error_log('Sams Hardware Debug - Product ID ' . $product->get_id() . ': Active taxonomy = ' . ($active_taxonomy ?: 'NONE'));
    
    if ($active_taxonomy) {
        $terms = get_the_terms($product->get_id(), $active_taxonomy);
        error_log('Sams Hardware Debug - Product ID ' . $product->get_id() . ': Terms = ' . print_r($terms, true));
        if ($terms && !is_wp_error($terms) && !empty($terms)) {
            $brand = $terms[0]->name;
            error_log('Sams Hardware Debug - Product ID ' . $product->get_id() . ': Brand from taxonomy = ' . $brand);
        }
    }
    ...
    if (!$brand) {
        ...
        if ($brand_attr) {
            $brand = $brand_attr;
            error_log('Sams Hardware Debug - Product ID ' . $product->get_id() . ': Brand from attribute = ' . $brand);
        }
    }
    
    error_log('Sams Hardware Debug - Product ID ' . $product->get_id() . ': Final brand = ' . ($brand ?: 'NULL'));
...
function get_products_secure($request) {
    ...
    $cache_key = 'sams_hardware_products_' . md5(serialize($params));
...
function get_product_secure($request) {
    ...
    // Check cache first
    $cache_key = 'sams_hardware_product_' . $product_id;
    $cached = get_transient($cache_key);
    if ($cached !== false) {
        return $cached;
    }
...
function get_linked_products_secure($request) {
    ...
    // Check cache first
    $cache_key = 'sams_hardware_linked_products_' . $product_id;
    $cached = get_transient($cache_key);
    if ($cached !== false) {
        return $cached;
    }
...
function get_brands_secure($request) {
    ...
    // Create cache key
    $cache_key = 'sams_hardware_brands_' . md5(serialize($params));
...
function get_gallery_media_secure($request) {
    ...
    $cache_key = 'sams_hardware_gallery_media_' . md5(json_encode(array(
        'page' => $page,
        'per_page' => $per_page,
...
function get_categories_secure($request) {
    ...
    // Create cache key
    $cache_key = 'sams_hardware_categories_' . md5(serialize($params));
...
function get_child_categories_secure($request) {
    ...
    $cache_key = 'sams_hardware_child_categories_' . md5(json_encode(array(
        'parent_id' => $parent_id,
...
function create_order_secure($request) {
    ...
    // Delivery option (Delivery Options by KaizenTech)
...
function update_order_secure($request) {
    ...
    error_log("Sams Hardware Debug: update_order_secure called for Order ID: {$order_id} with data: " . json_encode($update_data));
    
    $order = wc_get_order($order_id);
    
    if (!$order) {
        error_log("Sams Hardware Debug: Order not found: {$order_id}");
        return new WP_Error('order_not_found', 'Order not found', array('status' => 404));
    }
    
    // Update status if provided
    if (isset($update_data['status'])) {
        error_log("Sams Hardware Debug: Updating status to: {$update_data['status']}");
...
function clear_product_cache_on_stock_change($meta_id, $post_id, $meta_key, $meta_value) {
    ...
    if ($meta_key === '_stock' || $meta_key === '_stock_status') {
        ...
        delete_transient('sams_hardware_product_' . $post_id);
        
        // Clear the products list cache (since stock affects product listings)
        global $wpdb;
        $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '_transient_sams_hardware_products_%'");
        
        // Log the cache clearing
        error_log("Sams Hardware Debug: Product cache cleared for stock change on product {$post_id}");
    }
}
...
add_action('admin_menu', 'sams_hardware_add_debug_menu');

function sams_hardware_add_debug_menu() {
    add_menu_page(
        'Sams Hardware Debug Logs',
        'Debug Logs',              // Menu title
        'manage_options',          // Capability
        'sams-hardware-debug-logs',       // Menu slug
        'sams_hardware_debug_logs_page',
        'dashicons-editor-code',   // Icon
        100                        // Position
    );
}

function sams_hardware_debug_logs_page() {
    ?>
    <div class="wrap">
        <h1>🔍 Sams Hardware Debug Logs</h1>
    ...
    function refreshLogs() {
        ...
        fetch('<?php echo rest_url('sams-hardware/v1/debug/logs'); ?>', {
            headers: {
                'X-API-Key': 'sams-hardware-react-2024'
            }
        })
        .then(r => r.json())
        .then(data => {
    ...
    function downloadLogs() {
        ...
        a.href = url;
        a.download = `sams-hardware-debug-logs-${new Date().toISOString().slice(0,10)}.txt`;
        a.click();
...
add_action('rest_api_init', function () {
    // User Registration
    register_rest_route('sams-hardware/v1', '/auth/register', array(
        'methods' => 'POST',
        'callback' => 'sams_hardware_handle_user_registration',
        'permission_callback' => '__return_true', // Public endpoint
    ));
    
    // User Login
    register_rest_route('sams-hardware/v1', '/auth/login', array(
        'methods' => 'POST',
        'callback' => 'sams_hardware_handle_user_login',
        'permission_callback' => '__return_true', // Public endpoint
    ));
    
    // Validate Token (check if user is logged in)
    register_rest_route('sams-hardware/v1', '/auth/validate', array(
        'methods' => 'POST',
        'callback' => 'sams_hardware_validate_auth',
        'permission_callback' => '__return_true',
    ));
    
    // Get Current User Profile
    register_rest_route('sams-hardware/v1', '/user/profile', array(
        'methods' => 'GET',
        'callback' => 'sams_hardware_get_user_profile',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
    
    // Update User Profile
    register_rest_route('sams-hardware/v1', '/user/profile', array(
        'methods' => 'PUT',
        'callback' => 'sams_hardware_update_user_profile',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
    
    // Get User Orders
    register_rest_route('sams-hardware/v1', '/user/orders', array(
        'methods' => 'GET',
        'callback' => 'sams_hardware_get_user_orders',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
    
    // Logout
    register_rest_route('sams-hardware/v1', '/auth/logout', array(
        'methods' => 'POST',
        'callback' => 'sams_hardware_handle_user_logout',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
});
 ...
 */
function sams_hardware_handle_user_registration($request) {
    ...
    // Log registration
    error_log("Sams Hardware Debug: New user registered - ID: {$user_id}, Email: {$email}");
    
    // Get user data for response
    $user = get_userdata($user_id);
    
    // Create auth token (using WordPress nonces for now)
    $token = wp_create_nonce('sams_hardware_auth_' . $user_id);
 ...
 */
function sams_hardware_handle_user_login($request) {
    $params = $request->get_json_params();
    ...
    if (is_wp_error($user)) {
        error_log("Sams Hardware Debug: Login failed for {$email} - " . $user->get_error_message());
    ...
    error_log("Sams Hardware Debug: User logged in - ID: {$user->ID}, Email: {$email}");
    
    // Create auth token
    $token = wp_create_nonce('sams_hardware_auth_' . $user->ID);
 ...
 */
function sams_hardware_validate_auth($request) {
    // Use the same permission check logic
    $is_authenticated = sams_hardware_check_user_logged_in($request);
 ...
 * Get User Profile
 */
function sams_hardware_get_user_profile($request) {
    $user_id = get_current_user_id();
 ...
 * Update User Profile
 */
function sams_hardware_update_user_profile($request) {
    $user_id = get_current_user_id();
 ...
 * Get User Orders
 */
function sams_hardware_get_user_orders($request) {
    $user_id = get_current_user_id();
 ...
 * Logout Handler
 */
function sams_hardware_handle_user_logout($request) {
    $user_id = get_current_user_id();
    
    if ($user_id) {
        error_log("Sams Hardware Debug: User logged out - ID: {$user_id}");
    }
 ...
 */
function sams_hardware_check_user_logged_in($request) {
 ...
 */
add_action('woocommerce_checkout_order_processed', 'sams_hardware_link_order_to_user', 10, 1);

function sams_hardware_link_order_to_user($order_id) {
    $user_id = get_current_user_id();
    ...
    if ($user_id) {
        ...
        if ($order && $order->get_customer_id() === 0) {
            ...
            $order->save();
            error_log("Sams Hardware Debug: Order {$order_id} linked to user {$user_id}");
        }
    }
}
...
add_action('rest_api_init', function () {
    // Get user points
    register_rest_route('sams-hardware/v1', '/gamification/points', array(
        'methods' => 'GET',
        'callback' => 'sams_hardware_get_user_points',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
    
    // Get points history
    register_rest_route('sams-hardware/v1', '/gamification/points/history', array(
        'methods' => 'GET',
        'callback' => 'sams_hardware_get_points_history',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
    
    // Get user rank
    register_rest_route('sams-hardware/v1', '/gamification/rank', array(
        'methods' => 'GET',
        'callback' => 'sams_hardware_get_user_rank',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
    
    // Get user achievements
    register_rest_route('sams-hardware/v1', '/gamification/achievements', array(
        'methods' => 'GET',
        'callback' => 'sams_hardware_get_user_achievements',
        'permission_callback' => 'sams_hardware_check_user_logged_in',
    ));
});
 ...
 */
function sams_hardware_get_user_points($request) {
    $user_id = get_current_user_id();
    ...
    if (function_exists('gamipress_get_user_points')) {
        ...
        if (!empty($point_types)) {
            ...
            $possible_slugs = array('sams-hardware-points', 'sams_hardware_points', 'points', 'point');
            ...
            if ($point_type_slug) {
                ...
                error_log("Sams Hardware Debug: Fetched {$points} points for user {$user_id} using point type '{$point_type_slug}'");
            } else {
                error_log("Sams Hardware Debug: No point types found in GamiPress");
            }
        } else {
            error_log("Sams Hardware Debug: GamiPress point types not available");
        }
    } else {
        error_log("Sams Hardware Debug: GamiPress function gamipress_get_user_points not available");
    }
 ...
 */
function sams_hardware_get_points_history($request) {
    $user_id = get_current_user_id();
 ...
 */
function sams_hardware_get_user_rank($request) {
    $user_id = get_current_user_id();
    ...
    if (function_exists('gamipress_get_rank_types')) {
        ...
        if (!empty($rank_types)) {
            ...
            $possible_slugs = array('sams-hardware-rank', 'sams_hardware_rank', 'rank', 'ranks');
            $rank_type_slug = null;
            ...
            if ($rank_type_slug && function_exists('gamipress_get_user_rank')) {
                ...
                if ($rank) {
                    ...
                    error_log("Sams Hardware Debug: Fetched rank '{$rank->post_title}' for user {$user_id} using rank type '{$rank_type_slug}'");
                } else {
                    error_log("Sams Hardware Debug: No rank found for user {$user_id} in rank type '{$rank_type_slug}'");
                }
            }
        } else {
            error_log("Sams Hardware Debug: No rank types found in GamiPress");
        }
    } else {
        error_log("Sams Hardware Debug: GamiPress rank functions not available");
    }
 ...
 */
function sams_hardware_get_user_achievements($request) {
    $user_id = get_current_user_id();
    ...
    if (function_exists('gamipress_get_achievement_types')) {
        ...
        if (!empty($achievement_types)) {
            ...
            error_log("Sams Hardware Debug: Fetched {$achievements_data['total_earned']} earned and " . count($achievements_data['unearned']) . " unearned achievements for user {$user_id}");
        } else {
            error_log("Sams Hardware Debug: No achievement types found in GamiPress");
        }
    } else {
        error_log("Sams Hardware Debug: GamiPress achievement functions not available");
    }
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('sams-hardware/v1', '/gamification/leaderboard', array(
        'methods' => 'GET',
        'callback' => 'sams_hardware_get_leaderboard',
        'permission_callback' => '__return_true', // Public endpoint
    ));
});
 ...
 */
function sams_hardware_get_leaderboard($request) {
    global $wpdb;
    ...
    // Get point type slug (same logic as sams_hardware_get_user_points)
    $point_type_slug = null;
    $point_types = array();
    ...
    foreach ($users as $user_data) {
        ...
        if (function_exists('gamipress_get_rank_types')) {
            ...
            if (!empty($rank_types)) {
                $rank_type_slug = null;
                $possible_rank_slugs = array('sams-hardware-rank', 'sams_hardware_rank', 'rank', 'ranks');
    ...
    // Log for debugging
    error_log("Sams Hardware Debug: Leaderboard fetched - {$point_type_slug}, " . count($leaderboard) . " users");
